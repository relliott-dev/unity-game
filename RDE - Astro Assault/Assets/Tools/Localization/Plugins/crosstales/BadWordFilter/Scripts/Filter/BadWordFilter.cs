...
namespace Crosstales.BWF.Filter
{
   ...
   public class BadWordFilter : BaseFilter
   {
         ...
         if (isReady)
         {
            ...
            else
            {
               ...
               if (SimpleCheck)
               {
                  ...
                  {
                     _text = System.Text.RegularExpressions.Regex.Replace(_text, badword, Helper2.CreateString(ReplaceCharacters, badword.Length), System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                     hasBadWords = true;
                  }
      ...
      private string replaceCapture(string text, System.Text.RegularExpressions.Capture capture, bool markOnly, string prefix, string postfix, int offset)
      {
         ...
         string replacement = markOnly ? prefix + capture.Value + postfix : prefix + Helper2.CreateString(ReplaceCharacters, capture.Value.Length) + postfix;
